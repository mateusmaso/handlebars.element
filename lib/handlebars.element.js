(function(Handlebars) {
  ...
  Handlebars.parseValue = function(value, bool) {
    ...
    return bool ? value !== false : (value === "" ? undefined : value);
  };
  ...
  Handlebars.parseHTML = function(html) {
    ...
    while (nodes.length != 0) {
      ...
      for (var index = 0; index < nodes.length; index++) {
        var binding = {owner: nodes[index], element: null, attributes: []};
    ...
    for (var index = 0; index < bindings.length; index++) {
      ...
      if (element) {
        ...
        for (var bIndex = 0; bIndex < element.attributes.length; bIndex++) {
          ...
          var bool = elementFn.options.booleans && elementFn.options.booleans.indexOf(name) >= 0;
